...
- **上游群 (UpstreamGroup)**：在群内绑定一个或多个接口 ID（`绑定接口 upstream_01`）后自动升级；解绑后同样回落为普通群
...
- **主要功能**（按优先级顺序）:
  ...
  3. **功能插件处理** (Feature Manager)：
     ...
     - 已实现的功能插件：
      ...
      - **接口 ID 管理**（优先级 16）：解析“绑定接口 upstream_01”/“解绑接口 [接口ID]”等命令，可维护多接口列表，仅在普通/上游群启用
      - **四方支付查询**（优先级 25）：显式指令（如 `余额`）与自动订单查单
