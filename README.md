...
- **群组分级**：
  ...
  - **上游群 (UpstreamGroup)**：绑定一个或多个接口 ID 后自动升级，同样与商户群互斥
  - 绑定/解绑商户号或接口 ID 均需 Admin+，所有操作会写入审计日志
...
| `绑定接口 [接口ID]` / `解绑接口 [接口ID]` / `接口ID` | Admin+ | 管理上游接口 ID，可重复绑定多个，不带参数的 `解绑接口` 会清空全部 |
| `上游账单` / `上游账单 upstream_01 10月26` | 上游群成员 | 针对绑定的接口 ID 查询上游跑量/实收/代理收益/订单数；多接口时需指定接口 ID，可附带日期（默认当天，北京时间），基于 `/summarybydaypzid` |
...
- **数据库设计**：
  ...
  **groups Collection**（群组信息表）
  ...
  - `settings` - 群组功能配置（计算器、支付查询、自动查单、USDT 价格、渠道转发、记账开关、商户号、接口 ID 等）
