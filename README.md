...
- **配置项**：
  ...
  - 自动查单增强（可选）：
    - `XAI_API_KEY` - xAI API Key，配置后启用 LLM 辅助提取复杂文本中的订单号
    - `XAI_BASE_URL` - xAI API 地址，默认 `https://api.x.ai/v1`
    - `XAI_MODEL` - 指定使用的 xAI 模型，默认 `grok-beta`
    - `XAI_TIMEOUT_SECONDS` - xAI 请求超时时间（秒，默认 `15`）
...
| `通道账单` / `通道账单10月26` | 所有成员 | 按通道列出跑量、成交、笔数，并附带提款明细与余额（默认当天，可指定日期，基于北京时间） |
| `费率` | 所有成员 | 展示四方支付通道开关与费率（过滤测试通道，按 API 顺序原样列出） |
...
| （自动）订单查单 | 无 | 在群聊文本、媒体标题/文件名中检测 6 位以上的订单号片段，自动补全商户前缀后查询四方支付订单（每条消息最多 3 条），并返回订单状态、金额、平台订单号等详情 |

> ℹ️ **自动查单触发说明**
> - 仅在群组/超级群组中触发，且需要在 `/configs` 中启用「四方支付查询」并绑定商户号
> - 会优先使用商户号作为前缀补全订单号，不影响原始文本
> - 同一条消息（含文本、媒体标题或文件名）最多自动查询 3 条订单，避免刷屏
> - 若配置了 `XAI_API_KEY`，会通过 xAI 辅助识别截图说明、日志等非结构化文本中的订单号
