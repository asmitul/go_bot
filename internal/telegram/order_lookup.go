...
func (b *Bot) sendAutoOrderMessage(ctx context.Context, msg *botModels.Message, merchantID int64, original, composed string, order *paymentservice.Order) {
	message := buildAutoOrderMessage(merchantID, original, composed, order)
	b.sendMessage(ctx, msg.Chat.ID, message, msg.ID)
}

func buildAutoOrderMessage(merchantID int64, original, composed string, order *paymentservice.Order) string {
	var lines []string
	...
	if order == nil {
		lines = append(lines, "❌ 未查询到相关订单，请核对后重试。")
