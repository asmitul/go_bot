...
**说明**：面向平台视角的按日统计，仅依赖 `pzid` 和时间范围过滤，忽略调用者 `merchant_id`，因此可查看所有商户在指定上游配置下的支付表现（仅统计 `status>0` 的已支付订单）。

> ⚙️ 实际用途：Telegram 上游群的「上游账单」命令会调用该接口，读取绑定的接口 ID（`pzid`）在指定日期的汇总数据并回传跑量、商户实收、代理收益、订单数。
...
> 需要同时提交公共签名参数；`timestmap` 建议使用 10 位秒级时间戳。
